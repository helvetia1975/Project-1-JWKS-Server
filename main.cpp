...
//Function to convert bignum to a string
std::string bignum_to_raw_string(const BIGNUM *bn) {
    int bn_size = BN_num_bytes(bn); //retrieves number of bytes to represent bn
    std::string raw(bn_size, 0); //sets the string to the appropiate size
    BN_bn2bin(bn, reinterpret_cast<unsigned char *>(&raw[0])); //string is given the binary value of the bignum
    return raw;
}

//Get public key and extract it as a string
std::string extract_pub_key(EVP_PKEY *pkey) {
    BIO *bio = BIO_new(BIO_s_mem()); //holds the public key
    PEM_write_bio_PUBKEY(bio, pkey); //write the public key in PEM
    char *data = NULL;
    long len = BIO_get_mem_data(bio, &data); //retrieves data from BIO
    std::string result(data, len); //puts the result in a string
    BIO_free(bio); //releasing memory
    return result;
}

//Get private key and extract it as a string
std::string extract_priv_key(EVP_PKEY *pkey) {
    BIO *bio = BIO_new(BIO_s_mem()); //holds the private key
    PEM_write_bio_PrivateKey(bio, pkey, NULL, NULL, 0, NULL, NULL); //write the private key in PEM
    char *data = NULL;
    long len = BIO_get_mem_data(bio, &data); //retrieves data from BIO
    std::string result(data, len); //puts the result in a string
    BIO_free(bio); //releasing memory
...
//Turning binary characters into ASCII values
